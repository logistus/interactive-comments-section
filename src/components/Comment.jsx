...
function Comment({ data, type, currentUser }) {
  ...
  return (
    <>
      {showModal && <><Modal setShowModal={setShowModal} /><Dimmer /></>}
      <div className={`comment__wrapper ${type === 'reply' ? 'is-reply' : ''}`}>
        ...
        <div className={`comment ${type === 'reply' ? 'reply' : ''}`}>
          <div className="comment__vote">
            <a href="#" className="comment__vote__up">
            ...
            <a href="#" className="comment__vote__down">
          ...
          <div className="comment__body">
            {!showUpdate ? <div>
              ...
              {data.content}
            </div> : <form className="update-form">
              <textarea defaultValue={(data.replyingTo ? `@${data.replyingTo} ` : '') + data.content} rows={3}></textarea>
              <button type="submit" onClick={(e) => { e.preventDefault(); setShowUpdate(!showUpdate) }}>UPDATE</button>
            </form>}
          </div>
        </div>
      </div>
      {addReply ? <ReplyForm currentUser={currentUser} type={type} /> : ''}
      {data.replies ? data.replies.map(reply => <CommentReply data={reply} key={reply.id} currentUser={currentUser} />) : ''}
    </>
  );
}
